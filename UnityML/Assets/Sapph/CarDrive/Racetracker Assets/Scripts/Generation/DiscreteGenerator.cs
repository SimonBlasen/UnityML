...
public enum DiscreteGenerationMode
{
    PERFECT_PART_CIRCLES, BEZIER_SPLINES, COMBINED
}

public class OvercrossBetweening
{
    public Vector3 BetwLB { get; set; }
    public Vector3 BetwRB { get; set; }
    public Vector3 BetwLT { get; set; }
    public Vector3 BetwRT { get; set; }

    public OvercrossBetweening(Vector3 betwLB, Vector3 betwLT, Vector3 betwRB, Vector3 betwRT)
    {
        BetwLB = betwLB;
        BetwLT = betwLT;
        BetwRB = betwRB;
    ...
    public Vector3 Interpolate(float x, float y)
    {
        Vector3 goX = ((BetwRB - BetwLB) + (BetwRT - BetwLT)) * 0.5f * x;
        Vector3 goY = ((BetwLT - BetwLB) + (BetwRT - BetwRB)) * 0.5f * y;
        //Vector3 goY = (BetwLT - BetwLB) * (1f - x) + (BetwRT - BetwRB) * x;

        Vector3 res = BetwLB + goX + goY;

        return new Vector3(res.x, 0f, res.z);
    }
}
...
public class Circle
{
    ...
    public Circle()
    {
        ...
        Out = new Vector2(float.MinValue, float.MinValue);
        In = new Vector2(float.MinValue, float.MinValue);
    }

    public Circle(Vector2 midpoint, float radius, bool clockwise)
    {
        Midpoint = midpoint;
        Radius = radius;
        Clockwise = clockwise;
    ...
    public Circle(Vector2 midpoint, float radius, bool clockwise, Vector2 pointIn, Vector2 pointOut)
    {
        Midpoint = midpoint;
        Radius = radius;
        Clockwise = clockwise;
...
public class Utils
{
    public static Vector2 Intersect2D(Ray2D ray1, Ray2D ray2, out bool parallel)
    {
        Vector2 p1 = ray1.origin;
        Vector2 p2 = ray1.origin + ray1.direction;
        Vector2 p3 = ray2.origin;
        Vector2 p4 = ray2.origin + ray2.direction;
    ...
    public static bool PointRightTo(Ray2D ray, Vector2 point)
    {
        Ray2D rayToRight = new Ray2D(point, new Vector2(ray.direction.y, -ray.direction.x));

        bool parallel;
        Vector2 intersect = Intersect2D(ray, rayToRight, out parallel);

        if (!parallel)
        {
            float angleBetween = Vector2.Angle(rayToRight.direction, point - intersect);
    ...
    {
        int int1 = Random.Range(int.MinValue, int.MaxValue);
        int int2 = Random.Range(int.MinValue, int.MaxValue);

        long result = (int1 << 32) + (int2);
        return result;
    }
}